...
mask_for_black_colour_stream = cv2.VideoWriter(dir+'mask_for_black_colour.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (frame_width, frame_height))
eroded_mask_stream = cv2.VideoWriter(dir+'eroded_mask.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (frame_width, frame_height))
dilated_mask_stream = cv2.VideoWriter(dir+'dilated_mask.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (frame_width, frame_height))
bgr_mask_stream = cv2.VideoWriter(dir+'bgr_mask.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (frame_width, frame_height))
resized_frame_stream = cv2.VideoWriter(dir+'resized_frame.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (frame_width, frame_height))
...
while success:
    ...
    black_color_high = (255, 255, 130)
    mask_for_black_colour = cv2.inRange(hsv_frame, black_color_low, black_color_high)
    mask_for_black_colour_stream.write(mask_for_black_colour)
